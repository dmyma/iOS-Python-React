...
def customer_add_order(request):
    ...
    if request.method == "POST":
        # Get token
        access_token = AcessToke.objects.get(token = request.POST.get("access_token"),
            expires__gt = timezone.now())

        # Get profile
        customer = access_token.user.customer_add_order
...
def driver_pick_order(request):
    ...
    if request.method == "POST":
        ...
        if Order.objects.filter(driver = driver).exclude(status = Order.ONTHEWAY):
            return JsonResponse({"status": "failed", "error": "You can only pick one order at the sae time"})
...
def driver_get_revenue(request):
    ...
    revenue = {}
    today = timezone.now()
    current_weekdays = [today + timezone(days = i) for i in range(0 - today.weekday(), 7 - today.weekday())]
...
def driver_update_location(request):
    ...
    if request.method == "POST":
        ...
        driver.locaiton = request.POST["location"]
        driver.save()
